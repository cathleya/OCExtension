...
#define COLOR_LINK RGBHEX(0x507DAF)

#define IMAGE_MAX_SIZE 28
#define IMAGE_SIZE_OFFSET 8// 图片大小和字体大小的校正值
...
CGSize NISizeOfAttributedStringConstrainedToSize(NSAttributedString* attributedString, CGSize constraintSize, NSInteger numberOfLines) {
 ...
 //+0.5 解决有换行时 无法显示问题
  return CGSizeMake(ceilf(newSize.width + 0.5), ceilf(newSize.height + 0.5));
}
...
- (CGSize)boxSize {
    ...
    if (size.height > _maxWidth)
    {
        size.height = _maxWidth;
        size.width = size.height * ratio;
    }
    
    return CGSizeMake(size.width + self.margins.left + self.margins.right,
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
  if (self.touchedLink) {
    ...
    [self setNeedsDisplay];
      ...
      if ([self.delegate respondsToSelector:@selector(attributedLabelTouched:)]) {
          [self.delegate attributedLabelTouched:self];
      }
    ...
    for (NIAttributedLabelImage *labelImage in self.images) {
      ...
      if (NULL != delegate) {
        ...
        [attributedString insertAttributedString:space atIndex:labelImage.index];
          ...
          //删除多余的占位符
          NSUInteger iPos = labelImage.index+1;
          if (iPos < ([attributedString length]))
          {
              NSRange range = NSMakeRange(iPos, 1);
              NSString* str = [attributedString.string substringWithRange:range];
              if ([str isEqualToString:objectReplacementString])
              {
                  [attributedString deleteCharactersInRange:NSMakeRange(iPos, 1)];
              }
    ...
    for (CFIndex k = 0; k < runCount; k++) {
      ...
      switch (labelImage.verticalTextAlignment) {
        ...
        case NIVerticalTextAlignmentBottom:
              if (imageBoxHeight < lineHeight) {
                  imageBoxOriginY = lineBottomY + 1.5;
              }
              else
              {
...
- (NSArray *)accessibleElements {
  ...
  for (NSTextCheckingResult* result in allLinks) {
    ...
    if (IS_ARRAY(rectsForLink)
        || [rectsForLink count] == 0) {
      continue;
    }
...
CGFloat NIImageDelegateGetWidthCallback(void* refCon) {
  NIAttributedLabelImage *labelImage = (__bridge NIAttributedLabelImage *)refCon;
  return labelImage.boxSize.width + labelImage.margins.left + labelImage.margins.right;
}
...
- (void)insertImage:(UIImage *)image atIndex:(NSInteger)index margins:(UIEdgeInsets)margins verticalTextAlignment:(NIVerticalTextAlignment)verticalTextAlignment {
  ...
  labelImage.verticalTextAlignment = verticalTextAlignment;
    labelImage.maxWidth = self.font.pointSize + IMAGE_SIZE_OFFSET;
  if (nil == self.images) {
